...
void ContadorNumNaturais::processaLinha(const std::string &str) {
  ...
  string soma = 0;
  std::regex regularExpr("(\\+|-)?[0-9]+\\s*\\d+)");
  auto begin = std::sregex_iterator(str.begin(), str.end(), regularExpr);
  auto end = std::sregex_iterator();


  for (std::sregex_iterator iter(std::begin(str), std::end(str), regularExpr); iter != end; ++iter){
    processaLinha(str);
    linhaValida(str);
    soma = soma + iter->str();
  }
...
void InversorDeFrases::processaLinha(const std::string &str) {
  // TODO: Implemente este metodo:
  std::cout << "Imprime algo aqui!" << std::endli;
}
